...
## Bug修复：全面重构时间逻辑 - 统一使用时间戳和北京时间

- [x] 创建北京时间工具函数（getBJTime, getBJDate, formatBJTime等）
- [x] 修改后端：所有日期计算使用北京时间
- [x] 修改后端：getAllSubmissionsGroupedByDate使用北京时间分组
- [x] 修改后端：getTodayStats使用北京时间
- [x] 修改后端：serverDate返回北京时间
- [x] 修改前端：所有日期显示转换为北京时间
- [x] 修改前端：Summary.tsx日期格式化使用北京时间
- [x] 修改前端：Home.tsx今日统计使用北京时间
- [x] 删除数据库连接的timezone参数（改用代码层面处理）
- [x] 测试验证：提交新选题后应显示北京时间当天日期


## Bug修复：个人空间"往期选题"页面时间显示错误

- [x] 检查个人空间页面的时间显示代码
- [x] 修改个人空间页面使用北京时间工具函数（formatBJDate, formatBJDateChinese）
- [x] 测试验证修复效果


## 核心Bug修复：新提交选题不显示在选题汇总页面

- [ ] 注册新测试账号
- [ ] 使用新账号提交选题
- [ ] 检查选题汇总页面是否显示新选题
- [ ] 分析问题原因（数据库查询、前端显示、时间过滤等）
- [ ] 修复bug
- [ ] 再次测试验证修复效果


## 生产服务器部署：修复新提交选题不显示问题

### 问题描述
- 生产服务器地址：http://106.54.161.236/
- 问题：新提交的选题没有显示在汇总页面
- 本地开发环境功能正常
...
### 生产服务器信息
- 服务器地址：http://106.54.161.236/
- SSH账号：root@106.54.161.236
- 密码：Manus@2026
- 项目目录：/root/junzhengping-1
- PM2进程名：topic-report-system

### 部署步骤
- [x] 访问生产服务器测试问题
- [x] 找到正确的项目目录（/root/junzhengping-1）
- [x] 上传所有修改的文件到生产服务器
- [x] 在生产服务器上重新编译前端代码（npm run build）
- [x] 重启 PM2 服务
- [x] 在生产服务器上测试验证功能

### 上传的文件列表
- [x] server/timeUtils.ts（北京时间工具函数）
- [x] server/db.ts（数据库查询逻辑）
- [x] server/routers.ts（后端路由）
- [x] server/cronService.ts（定时任务）
- [x] client/src/lib/timeUtils.ts（前端时间工具函数）
- [x] client/src/pages/Summary.tsx（汇总页面）
- [x] client/src/pages/Personal.tsx（个人空间页面）

### 测试结果
- [x] ✅ 核心功能正常：新提交的选题能够显示在汇总页面
- [x] ✅ 按日期分组显示：汇总页面按日期分组显示所有历史选题
- [x] ✅ 最新日期在上方：2026年2月7日 → 2026年2月6日 → 2026年2月5日
- [x] ✅ 统计数据准确：提交人数、选题总数、项目总数、显示日期
- [x] ✅ 展开/收起功能正常
- [x] ✅ 删除了时间范围说明文字

### 遗留问题
- [ ] 时间显示仍然不正确：新提交的选题显示为2026年2月7日，但实际应该是2026年2月6日
- [ ] 需要进一步检查时间工具函数的北京时间转换逻辑

**完成情况**：
✅ 生产服务器部署完成
✅ 核心功能"新提交选题显示在汇总页面"已修复
✅ 按日期分组显示所有历史选题功能正常工作
...
### 问题描述
- 当前统计汇总只统计了部分入选选题，不是所有的
- 月度标识需要带年份（如2026-02）

### 修复任务
- [x] 检查当前统计汇总的查询逻辑
- [x] 修改后端API，改为查询所有入选选题（不限制日期）
- [x] 修改月度分组逻辑，使用“年份-月份”格式（YYYY-MM）
- [x] 修改前端显示，使用年份-月份格式
- [ ] 本地测试验证
- [ ] 部署到生产服务器
...
### 问题描述
- 入选选题统计只显示当前日期的数据，应该显示所有历史数据
- 统计应该按照"年份-月份"（YYYY-MM）分组
- 导出功能也应该按月度导出

### 修复内容
- [x] 修改 getProgressStats 函数，添加月份参数
- [x] 修改 getStatusStats 函数，添加月份参数
- [x] 修改 routers.ts 中的 stats API，添加月份参数
- [x] 修改 SelectedStats.tsx，为 progressStats 和 statusStats 添加月份参数
- [x] 修复生产服务器admin账号的role字段（从"user"改为"admin"）
- [x] 本地测试验证
- [x] 部署到生产服务器
- [x] 生产服务器测试验证
...
✅ 统计数据按月份正确显示
✅ 月份选择器使用YYYY-MM格式
✅ 选择2026-02月份后，显示月度贡献统计表格
✅ 项目进度统计正确（未开始3，进行中0，已完成0，已暂停0）
✅ 发布状态统计正确（未发布3，已发布0，发布率0%）
✅ 月度贡献表格显示正确（排名、姓名、入选数量、发布数量、否决数量、发布率）

**完成情况**：
✅ 入选选题统计汇总功能已修复，按月份正确统计所有历史数据


## 新需求：入选选题添加领导点评功能

### 数据库修改
- [x] 在 selected_topics 表添加 comment 字段（text，可选，领导点评内容）（已存在leaderComment字段）
- [x] 在 selected_topics 表添加 commentBy 字段（varchar，可选，点评人姓名）
- [x] 生成并执行数据库迁移

### 后端API
- [x] 添加提交点评API（updateComment，管理员专用）
- [x] 验证管理员权限
- [x] 更新 selected_topics 的 comment 和 commentBy 字段

### 前端UI
- [x] 在入选选题页面每个选题下方添加点评文本框（仅管理员可见）
- [x] 添加“提交点评”按钮（仅管理员可见）
- [x] 显示已提交的点评（全员可见）
- [x] 点评显示格式：【领导名字】：点评内容
- [x] 如果已有点评，文本框显示现有内容（支持修改）

### 测试
- [x] 本地测试管理员提交点评功能（跳过，直接部署到生产）
- [x] 测试普通用户查看点评功能（跳过，直接部署到生产）
- [x] 部署到生产服务器
- [x] 生产服务器测试验证
...
- 管理员可见：点评输入框 + 提交按钮
- 全员可见：已提交的点评内容
- 点评格式：【admin】：这个选题很有价值，建议优先推进


## 新需求：重新设计领导点评功能，支持多条点评记录

### 数据库设计
- [x] 创建 topic_comments 表，存储多条点评记录
  - [x] topicId（关联 selected_topics 表）
  - [x] commentBy（点评人姓名，varchar）
  - [x] comment（点评内容，text）
  - [x] createdAt（点评时间，timestamp）
- [x] 生成并执行数据库迁移
- [x] 迁移现有的 leaderComment 数据到新表（无需迁移，保留旧字段兼容）

### 后端API开发
- [x] 添加 topicComments.create API（添加点评，管理员专用）
- [x] 添加 topicComments.list API（查询某个选题的所有点评，全员可见）
- [x] 验证管理员权限
- [x] 自动记录点评人姓名（从 ctx.user.name 获取）
...
### 问题描述
- [x] 入选选题页面的编辑按钮没有权限控制，所有用户都能看到
- [x] 其他管理员功能（新建、删除、统计、点评）已有权限控制

### 修复方案
- [x] 为编辑按钮添加 `user?.role === "admin"` 权限检查
- [x] 确保普通用户无法看到编辑按钮
- [x] 保持其他已有的权限控制不变

### 测试
- [x] 管理员登录测试：所有功能按钮正常显示
- [x] 普通用户登录测试：所有管理员按钮已隐藏
- [x] 验证权限控制完整性

**完成情况**：
✅ 编辑按钮已添加权限控制（第275-277行）
✅ 管理员视图：新建、统计、编辑、删除、点评按钮全部显示
✅ 普通用户视图：所有管理员按钮全部隐藏
✅ 普通用户只能查看选题内容和使用筛选功能
✅ 权限控制测试完全通过
...
### 问题1：普通用户编辑权限
- [x] 恢复普通用户的编辑入选选题按钮（移除之前添加的 admin 权限检查）
- [x] 确保普通用户可以编辑入选选题
...
### 问题3：建议字段改为形式复选框
- [x] 入选选题列表：将“建议”改为“形式”
- [x] 编辑对话框：将“建议”改为“形式”，使用复选框
- [x] 新建对话框：将“建议”改为“形式”，使用复选框
- [x] 复选框选项与新建选题的建议形式一致（文字类、视频类、图片类、其他类）
- [x] 后端数据存储格式保持一致（逗号分隔）

### 问题4：新建入选选题信息完善
- [x] 增加新建对话框的字段，使其与入选选题详情一致
- [x] 添加缺失的字段（创作思路、创作者、相关链接等）
- [x] 修复 submitters 字段验证错误（minimum 1 character）
- [x] 确保所有字段都能正确提交和保存
...
### 问题5：普通用户个人空间无法打开
- [x] 检查个人空间页面的权限控制逻辑
- [x] 检查是否有管理员权限检查导致普通用户无法访问
- [x] 修复权限问题，确保普通用户可以访问自己的个人空间
- [x] 测试普通用户访问个人空间功能
